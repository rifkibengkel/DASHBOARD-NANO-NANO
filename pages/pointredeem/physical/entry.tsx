...
import { detailWnr } from "@/pages/api/pointredeem/list";
...
const ModalShipment = dynamic(
  () => import("../../../pageComponents/Winners/ShipmentMethod"),
...
const DataEntry = (props: any) => {
  ...
  const submitData = async () => {
    ...
    // console.log(dataTable, "321231");
    // console.log(entryCondition, "qweqwe");
  ...
  const approveData = async (status: string) => {
    // console.log(status, "22222222222222222222");
  ...
  const approveData100 = async (status: string) => {
    // console.log(status, "1111111111111111111111111111");
...
export const setShipMethod = async (data: any) => {
  let res: any = await fetch(`/api/pointredeem/setShip`, {
    method: "POST",
    body: JSON.stringify({
      data: data,
    }),
...
export const getServerSideProps: GetServerSideProps = withAuth(async (ctx) => {
  ...
  const masterPrizeSAP = await axios
    .get(`${jotf}/api/auth/master-prize/all/${programId[0].value}`, {
      headers: {
  ...
  // const dataDetail: any = await detailWnr(entryId);
  // console.log(dataDetail, "aaaaaaaaaaaaaaaaaa");
