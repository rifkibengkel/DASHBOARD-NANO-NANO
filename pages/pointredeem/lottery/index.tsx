  ...
  return (
    <>
      <SWRConfig value={{ fallback }}>
        <PageHeader
          title={`Point Redeem (Kupon Undian)`}
          extra={[
            <Row key="1">
...
const exportWinner = async (data: any) => {
  await window.open(
    `/api/pointredeem/export?mode=lottery&startDate=${data.startDate}&endDate=${data.endDate}&prize=${data.prize}&key=${data.key}&isValid=${data.isValid}&column=${data.columns}&direction=${data.directions}&isApproved=${data.isApproved}&isHaveAtt=${data.isHaveAtt}&prizeId=${data.prizeId}`
  );
};
