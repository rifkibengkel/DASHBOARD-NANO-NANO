...
import {masterDistrictCity, masterPrizes, masterRole} from "../../api/master/index";
...
import { titleCase } from "@/lib/clientHelper";
...
const Winners = (props: any, {fallback}: any) => {
  ...
  const handleSubmitImage = async (data: any) => {
    ...
    axios
      .post(`/api/winners/upload`, {file: base64File, entriesId: data.pickedId, userId: data.userId, wType: router.query.type})
      .then((res) => {
        if (res.status === 200) {
          setStates({selectedFile: null});
          if (states?.dataDetail?.img) {
            setStates({
              dataDetail: {
                img: [...states.dataDetail.img, {url: `api/images3/${res.data.data.url}`, id: res.data.data.id}]
              }
            })
          }
          Modal.success({
  ...
  const expWinner = async () => {
    ...
    await exportWinner(param, router.query.type);
  };
  ...
  const handleOpenModal = async (param: any) => {
    ...
    if (param.name === "openImage" && param.id) {
      const response = await getImage(
        router.query.type == "user" ? param.id : param.winnerId, 
        router.query.type
      );
      const data = {
  ...
  const getImage = useCallback(async (data: any, type: any) => {
    let res = await fetch(`/api/winners/images/?id=${data}&type=${type}`);
    if (res.status !== 404) {
      let dataList = await res.json();
      return dataList;
    } else {
  ...
  let tbc = {
      ...
      render: (text: any, record: any) =>
        ...
        {states.master.isAdmin ?
          ...
          <Tooltip title="Modify">
            <Button
              disabled={record.is_push == 0 ? true : record.status !== 0 ? true : record.is_approved === 0 ? false : true}
            >
              <Link href={`/winners/entry${router.query.type == "user" ? '' : 'Mode2'}?typeUser=${router.query.type}&entries=${record.id}&page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}&startDate=${states.filter.startDate}&endDate=${states.filter.endDate}&prize=${states.filter.prize}&key=${states.filter.key}&isValid=${states.filter.isValid}&isHaveAtt=${states.filter.isHaveAtt}`} passHref>
                Modify
              </Link>
            </Button>
          </Tooltip>
          ...
          {states.master.isAdmin ?
            ...
            <Button
              disabled={parseInt(record.approvalStep) < 1 ? true : record.is_push == 0 ? true : !record.ktp_name && router.query.type == "user" || !record.ktp_number && router.query.type == "user" ? true : record.isValid !== 0 ? true : record.is_approved === 1 || record.is_approved === 2 ? true : false}
            >
              <Link href={`/winners/entry${router.query.type == "user" ? '' : 'Mode2'}?typeUser=${router.query.type}&entries=${record.id}&aprv=${Buffer.from(JSON.stringify(1)).toString('base64')}&page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}&startDate=${states.filter.startDate}&endDate=${states.filter.endDate}&prize=${states.filter.prize}&key=${states.filter.key}&isValid=${states.filter.isValid}&isHaveAtt=${states.filter.isHaveAtt}`} passHref>
                Approve
              </Link>
            </Button>
          </Tooltip>
            ...
            <Tooltip title="Approve">
            <Button
              disabled={router.query.type == "user" ? true : parseInt(record.approvalStep) == 1 ? true : record.is_push == 0 ? true : !record.ktp_name && router.query.type == "user" || !record.ktp_number && router.query.type == "user" ? true : record.status !== 0 ? true : record.is_approved === 1 || record.is_approved === 2 ? true : false}
            >
              <Link href={`/winners/entry${router.query.type == "user" ? '' : 'Mode2'}?typeUser=${router.query.type}&entries=${record.id}&aprv=${Buffer.from(JSON.stringify(1)).toString('base64')}&page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}&startDate=${states.filter.startDate}&endDate=${states.filter.endDate}&prize=${states.filter.prize}&key=${states.filter.key}&isValid=${states.filter.isValid}&isHaveAtt=${states.filter.isHaveAtt}`} passHref>
                Approve
              </Link>
          ...
          {states.master.isAdmin && record.categoryId == 2 ? (
            <Tooltip title="Approve Shipment">
            <Button
              disabled={record.is_sent === 1 ? false : true}
            >
              <Link href={`/winners/entry${router.query.type == "user" ? '' : 'Mode2'}?typeUser=${router.query.type}&entries=${record.id}&aprv=${Buffer.from(JSON.stringify(2)).toString('base64')}&page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}&startDate=${states.filter.startDate}&endDate=${states.filter.endDate}&prize=${states.filter.prize}&key=${states.filter.key}&isValid=${states.filter.isValid}&isHaveAtt=${states.filter.isHaveAtt}`} passHref>
                Approve Shipment
              </Link>
            </Button>
          </Tooltip>
    ...
    title: `Winner List (${titleCase(router.query.type)})`,
...
const exportWinner = async (data: any, path: any) => {
  let res = await window.open(
    `/api/winners/export?startDate=${data.startDate}&endDate=${data.endDate}&prize=${data.prize}&key=${data.key}&status=${data.status}&column=${data.columns}&direction=${data.directions}&isHaveAtt=${data.isHaveAtt}&userType=${path}`
  );
};
...
export const getServerSideProps: GetServerSideProps = withAuth(async (ctx) => {
  ...
  const voucherMaster = await masterVoucher(2)
