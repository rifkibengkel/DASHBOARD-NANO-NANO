      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          if (states?.dataImages?.img) {
            setStates({
              dataImages: {
                img: [...states.dataImages.img, { url: `api/images3/${res.data.data.url}`, id: res.data.data.id }]
              }
            })
          }
          Modal.success({
      ...
      render: (text: any, record: any) =>
        record.prizeCat === 1 ? (
          <ButtonGroup>
            ...
            <Tooltip title="Re Topup">
            <Button
              disabled={(record.status === 1 || record.status === 2) || !record?.voucher ? true : false}
              onClick={() =>
          ...
          <ButtonGroup>
            ...
            <Tooltip title="Approve">
              ...
              >
                <Link href={`/winners/entry?entries=${record.id}&aprv=${Buffer.from(JSON.stringify(1)).toString('base64')}&page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}&startDate=${states.filter.startDate}&endDate=${states.filter.endDate}&prize=${states.filter.prize}&key=${states.filter.key}&isValid=${states.filter.isValid}&isHaveAtt=${states.filter.isHaveAtt}`} passHref>
                  Approve
                </Link>
              </Button>
            </Tooltip>
...
const exportWinner = async (data: any) => {
  let res = await window.open(
    `/api/winners/export?startDate=${data.startDate}&endDate=${data.endDate}&prize=${data.prize}&key=${data.key}&status=${data.status}&column=${data.columns}&direction=${data.directions}&isHaveAtt=${data.isHaveAtt}`
  );
};
