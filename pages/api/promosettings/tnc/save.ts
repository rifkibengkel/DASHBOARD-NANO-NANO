...
export const save = async (param: Input) => {
  ...
  const type = param.type ? Buffer.from(param.type, 'base64').toString('ascii') : "0"
  ...
  const insertList: any = await model.insertTNC(title, content, type.toString());
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
      ...
      const datas: any = await save(unmask)
