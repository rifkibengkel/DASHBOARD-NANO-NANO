...
export async function getData(param: SummaryDist) {
    ...
    let countDistributionPerProvince: any = await model.countDistributionByInput(key, media);

    for(let x = 0; x < countDistributionPerProvince.length; x++) {
        categories2.push(countDistributionPerProvince[x]?.regency_ktp)
        series2.push(Number(countDistributionPerProvince[x]?.total_submit))
    }

    let countDistribution: any = await model.countDistributionProvince(key, media);
    ...
    const listDistributor = await model.listDistributionProvince(key, media, column, direction.toUpperCase(), params.limit)
