...
export const countLists = (params: IParams) => {
  let countQuery = `SELECT COUNT(*) AS counts FROM white_list ${keyWhere(
    params.key
  )}`;
  return exeQuery(countQuery, []);
};

export const listWhitelists = (params: IParams) => {
  let listQuery = `SELECT id, name, sender,id_number FROM white_list${keyWhere(
    params.key
...
export const detailList = (id: string) => {
  let queryDetail = `SELECT * FROM white_list WHERE id = $1`;
  return exeQuery(queryDetail, [id]);
};

export const searchList = (sender: string, id: any, type: string) => {
  let edit = `SELECT COUNT(*) AS counts FROM white_list WHERE sender=$1 AND id!=$2`;
  let insert = `SELECT * FROM white_list WHERE sender= $1`;
  if (type == "edit") {
...
export const insertList = (
  ...
  let queryInsert = `INSERT INTO white_list(name,sender, id_number, "createdById") VALUES ($1,$2,$3,$4)`;
...
export const editList = (
  ...
  let editUserQuery = `UPDATE white_list SET name = $1, sender = $2, id_number = $3, "updatedById" = $4 WHERE id = $5`;
  return exeQuery(editUserQuery, [name, sender, idNumber, userId, id]);
};

export const deleteList = (id: string) => {
  let queryDelete = `DELETE FROM white_list WHERE id = $1`;
  return exeQuery(queryDelete, [id]);
};

export const updateUserId = (id: string, userId: string) => {
  return exeQuery(`UPDATE white_list SET "userId" = $1 WHERE id = $2`, [
    userId,
    id,
  ]);
};
