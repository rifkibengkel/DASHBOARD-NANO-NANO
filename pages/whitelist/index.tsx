...
import { getData } from "../api/whitelist/list";
...
const Whitelists = (props: any, { fallback }: any) => {
  ...
  const {
    ...
    `/api/whitelist/list?row=${states.data.dataPerPage}&page=${states.data.currentPage}&key=${states.filter.key}&columnSearch=${states.filter.columnSearch}&column=${states.filter.columns}&direction=${states.filter.directions}`
  );
  ...
  useEffect(() => {
    if (arrayWhitelist) {
      setStates({
        ...
        data: {
          ...states.data,
          dataPerPage: arrayWhitelist.dataPerPage,
          currentPage: arrayWhitelist.currentPage,
          totalData: arrayWhitelist.totalData,
          totalPage: arrayWhitelist.totalPage,
          list: arrayWhitelist.data,
      ...
      render: (text: any, record: any) => (
        <ButtonGroup>
          ...
          <Button
            ...
            shape="round"
            onClick={() =>
              showDeleteConfirm({ onOk: () => deleteWhitelist(record) })
            }
          >
            Delete
          </Button>
    ...
    <>
      <SWRConfig value={{ fallback }}>
        ...
        <Modals
          ...
          addNewList={addNewWhitelist}
          editList={modifyWhitelist}
        />
      </SWRConfig>
    </>
  );
...
export const getServerSideProps: GetServerSideProps = withAuth(async (ctx) => {
  ...
  return {
    props: {
      fallback: {
        "/api/whitelist/list": JSON.parse(JSON.stringify(data)),
      },
...
Whitelists.getLayout = function getLayout(page: ReactElement) {
