...
import { getData } from "../../api/promosettings/banner/list";
...
const ModalData = dynamic(
  () => import("@/pageComponents/PromoSettings/Banner/AddMod"),
  {
    loading: () => <p></p>,
  }
);
...
const Banner = (props: any, { fallback }: any) => {
  ...
  } = useSWR(`/api/promosettings/banner/list`);
  ...
  const handleSubmitData = async (data: any) => {
    ...
    axios
      .post(`/api/promosettings/banner/upload`, formData)
      .then((res) => {
        if (res.status === 200) {
  ...
  //     router.push(`/promosettings/banner/add?submit=${data}`)
  //   };

  const modifyData = async (param: any) => {
    const data = Buffer.from(JSON.stringify(param)).toString("base64");
    router.push(`/promosettings/banner/update?submit=${data}`);
  };

  const removeData = async (param: any) => {
    const data = Buffer.from(JSON.stringify(param)).toString("base64");
    router.push(`/promosettings/banner/delete?submit=${data}`);
  };
...
export const getServerSideProps: GetServerSideProps = withAuth(async (ctx) => {
  ...
  return {
    props: {
      fallback: {
        "/api/promosettings/banner/list": JSON.parse(JSON.stringify(data)),
      },
